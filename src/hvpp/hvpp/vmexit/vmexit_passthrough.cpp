...
namespace hvpp
{
    ...
    namespace detail
    {
        ...
        static bool is_syscall_instruction(const uint8_t* first, const uint8_t* last) noexcept
        {
            ...
            return static_cast<std::size_t>(last - first) >= std::size(opcode) && memcmp(first, opcode, sizeof(opcode))
                == 0;
        }
        ...
        case vmx::exit_qualification_mov_cr_t::access_from_cr:
            {
                switch (exit_qualification.cr_number)
                {
                case 3: gp_register = vp.guest_cr3().flags;
                    break;
    ...
    void vmexit_passthrough_handler::handle_mov_dr(vcpu_t& vp) noexcept
    {
        ...
        case vmx::exit_qualification_mov_dr_t::access_to_dr:
            ...
            {
            case 0: write<dr0_t>(dr0_t{gp_register});
                break;
            case 1: write<dr1_t>(dr1_t{gp_register});
                break;
            case 2: write<dr2_t>(dr2_t{gp_register});
                break;
            case 3: write<dr3_t>(dr3_t{gp_register});
                break;
            case 6: write<dr6_t>(vmx::adjust(dr6_t{gp_register}));
                break;
            case 7: vp.guest_dr7(vmx::adjust(dr7_t{gp_register}));
                break;
            default:
                break;
            }
        ...
        case vmx::exit_qualification_mov_dr_t::access_from_dr:
            switch (exit_qualification.dr_number)
            {
            case 0: gp_register = read<dr0_t>().flags;
                break;
            case 1: gp_register = read<dr1_t>().flags;
                break;
            case 2: gp_register = read<dr2_t>().flags;
                break;
            case 3: gp_register = read<dr3_t>().flags;
                break;
            case 6: gp_register = read<dr6_t>().flags;
                break;
            case 7: gp_register = vp.guest_dr7().flags;
                break;
            default:
                break;
            }
  ...
  if (exit_qualification.string_instruction)
  {
    ...
    void vmexit_passthrough_handler::handle_gdtr_idtr_access(vcpu_t& vp) noexcept
    {
        ...
        case vmx::instruction_info_gdtr_idtr_access_t::instruction_sgdt:
            ...
            memcpy(guest_va, &gdtr64, guest_in_long_mode()
                                          ? sizeof(gdtr64)
                                          : sizeof(gdtr32));
            break;
        ...
        case vmx::instruction_info_gdtr_idtr_access_t::instruction_sidt:
            ...
            memcpy(guest_va, &idtr64, guest_in_long_mode()
                                          ? sizeof(idtr64)
                                          : sizeof(idtr32));
            break;
            ...
            case exception_vector::invalid_opcode:
                {
                    ...
                    {
                        hvpp_trace(
                            "handle_interrupt (invalid_opcode) - read_guest_memory(%p, %u) failed, injecting #PF",
                            vp.context().rip,
                            instruction_length);
            ...
            case exception_vector::general_protection:
                {
                    ...
                    {
                        hvpp_trace(
                            "handle_interrupt (general_protection) - read_guest_memory(%p, %u) failed, injecting #PF",
                            vp.context().rip,
                            instruction_length);
